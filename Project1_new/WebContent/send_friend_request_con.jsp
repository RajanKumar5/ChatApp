...
<% 
	...
	if(session.getAttribute("email") == null || session.getAttribute("name") == null){
		...
		ArrayList<String> friendList = DB_Details.getFriendList(my_email);
		ArrayList<String> requestList = DB_Details.getRequestList(my_email);
		
		if(friendList.indexOf(friend_email) != -1){
			%>
				<script>
					alert("Can't send friend request. User exists in your friend list.");
				</script>
			<%
			RequestDispatcher rd = request.getRequestDispatcher("search");
			rd.include(request, response);
		}
		else if(requestList.indexOf(friend_email) != -1){
			%>
			<script>
				alert("Can't send friend request. You already have a friend request from this user.");
			</script>
			<%
			RequestDispatcher rd = request.getRequestDispatcher("search");
			rd.include(request, response);
			...
			try{		
				...
				String sql1 = "insert into " + send_table + " values(?, ?)";
				...
				stmt.setString(2, my_name);
				stmt.executeUpdate();
				 
				stmt1 = con.prepareStatement(sql1);
				stmt1.setString(1, friend_email);
				stmt1.setString(2, friend_name);
				stmt1.executeUpdate();
