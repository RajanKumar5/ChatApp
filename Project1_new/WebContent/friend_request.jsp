...
<html lang="en">
  <head>
    ...
    <title>Friend Request|ChatApp</title>
    <style type="text/css">
      #imgr{
        width:65px;
        height:65px;
...
<div class="container mt-5 pt-5">
  ...
  <thead>
    <tr class="text-center">
      <th colspan="3">Friend Request</th>
    </tr>
  </thead>
  <tbody>
  <% 
	...
	try{
		String table_name = uid + "_friendRequest";
		...
		while(rs.next()){
					...
					%>
				      ...
				      <td><input type="text" readonly="" name="name" class="inp" value="<%= name %>">
				      	...
				      	<form action="accept-friend-request" method="post">
				      		<button type="submit" class="btn btn-success btn-sm text-center">Accept</button>
				      		<input type="hidden" name="name"  value="<%= name %>">
				          	<input type="hidden" name="email" value="<%= friend_email%>">
				      	</form>
				      	<form action="decline-friend-request" method="post">
				      		<button type="submit" class="btn btn-danger btn-sm text-center">Decline</button>
				      		<input type="hidden" name="name"  value="<%= name %>">
				          	<input type="hidden" name="email" value="<%= friend_email%>">
				      	</form>
				      	</td>
  		...
  		if(f == 0){
  			%>
  				<tr class="text-center">
			      <th colspan="3">You don't have any friend requests.</th>
			    </tr>
  			<%	
  		}
	}
